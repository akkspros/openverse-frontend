...
export const Template = (args, { argTypes }) => ({
  template: `<AudioTrack :style="style" v-bind="$props" :audio="audioObj"/>`,
  components: { AudioTrack },
  props: Object.keys(argTypes).filter(
    (key) => !['style', 'class'].includes(key)
  ),
  data() {
...
The component can be rendered in a multiple different layouts, by specifying the
`layout` prop. The layout can further by customised by the `size` and `aspect`
props.

### Full layout

The full layout is always a full-width component. It does not support any
variants in size or aspect.
...
Available sizes are 's' and 'm'. This layout does not support aspect.
...
Available sizes are 'm' and 'l'. The size 'm' supports a 'wide' aspect.
